...
export default function Home() {
  ...
  const runSharpenPipeline = useCallback(
    async (file: File, blob: Blob, ultra: boolean) => {
      setSharpening(true);
      try {
        const url = await createHDDownloadUrl(file, blob, ultra);
        return url;
      } catch (err) {
    ...
    <main className="relative z-10 min-h-screen">
      ...
      <section className="px-4 pt-16 md:pt-24 pb-8 text-center">
        ...
        >
          ...
          <p className="mt-4 text-lg md:text-xl text-gray-500 max-w-xl mx-auto leading-relaxed">
            Remove image backgrounds instantly with AI. Upload any photo and get a
            sharp transparent PNG in seconds. No login, no limits.
          </p>
        </motion.div>
      </section>
        ...
        <div className="max-w-2xl mx-auto">
          ...
          <AnimatePresence mode="wait">
            {loading && (
              <motion.div
                ...
                animate={{ opacity: 1 }}
                exit={{ opacity: 0 }}
                className="glass rounded-2xl mt-6"
              >
                <LoadingSpinner />
              </motion.div>
            )}
          ...
          {error && (
            <motion.div
              ...
              animate={{ opacity: 1, y: 0 }}
              className="mt-6 p-4 bg-red-50 border border-red-100 rounded-xl text-red-600 text-sm text-center"
            >
              {error}
              <button onClick={handleReset} className="block mx-auto mt-2 text-red-700 underline hover:no-underline">
                Try again
              </button>
            </motion.div>
          )}
          ...
          {originalUrl && !loading && (
            ...
            >
              ...
              {processedUrl && !sharpening && (
                <>
                  <BeforeAfter original={originalUrl} processed={displayUrl || processedUrl} />
                  ...
                  >
                    <button
                      ...
                      className="gradient-btn px-8 py-3.5 text-base inline-flex items-center justify-center gap-2"
                    >
                    ...
                    <button
                      onClick={handleUltraHDToggle}
                      className={`px-6 py-3.5 text-base rounded-xl border-2 inline-flex items-center justify-center gap-2 transition-all duration-300 ${
                        ultraHD
                          ? "border-accent-400 bg-accent-50 text-accent-700 shadow-md shadow-accent-200/50"
                    ...
                    <button
                      onClick={handleReset}
                      className="px-6 py-3.5 text-base rounded-xl border border-gray-200 text-gray-600 hover:bg-gray-50 transition-colors inline-flex items-center justify-center gap-2"
                    >
                      <svg className="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
      ...
      {!originalUrl && (
        <motion.div
          initial={{ y: 100 }}
          animate={{ y: 0 }}
          className="fixed bottom-0 left-0 right-0 p-4 bg-white/90 backdrop-blur-lg border-t border-gray-100 md:hidden z-50"
        >
          <button
            onClick={() => {
            ...
            className="gradient-btn w-full py-3.5 text-sm"
          >
            Upload Image
          </button>
        </motion.div>
