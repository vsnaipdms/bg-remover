...
function buildStrongKernel(amount: number): number[] {
  const a = Math.max(0, Math.min(2, amount));
  return [
    -a, -a, -a,
    -a, 1 + 8 * a, -a,
...
function buildLaplacianEdge(amount: number): number[] {
...
function sharpenFromData(
  ...
  const kernel = strong ? buildStrongKernel(amount) : buildSharpenKernel(amount);
...
export async function sharpenProcessedImage(
  ...
  if (ultraHD) {
    const preSharpen = sharpenFromData(baseData.data, w, h, 0.5, false);
    ctx.putImageData(new ImageData(preSharpen, w, h), 0, 0);
    ...
    const clean = sharpenFromData(textured.data, w, h, 0.3, false);
    ...
    const preSharpen = sharpenFromData(baseData.data, w, h, 0.4, false);
    ctx.putImageData(new ImageData(preSharpen, w, h), 0, 0);
    ...
    const textured = ctx.getImageData(0, 0, w, h);
    const clean = sharpenFromData(textured.data, w, h, 0.25, false);
    const final = new Uint8ClampedArray(baseData.data.length);
    for (let i = 0; i < baseData.data.length; i += 4) {
      if (baseData.data[i + 3] < 5) {
        final[i] = 0;
